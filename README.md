...
<h1 align="center">PyGeoX</h1>
<p align="center">
  <strong>Internalizing Geometric Law: a programmable geometry language for precision-critical LLM generation</strong>
</p>

<p align="center">
  <!-- TODO: add arXiv link, e.g. <a href="https://arxiv.org/abs/XXXX.XXXXX">Paper (arXiv)</a> · -->
  <a href="docs/_build/html/index.html">Documentation</a>
  ·
  <a href="https://huggingface.co/datasets/rafaelcabral96/PyGeoX">Dataset (🤗 Hugging Face)</a>
  ·
...
These problems share one shape: **geometry constraint solving (GCS)**, finding a configuration of entities that satisfies dozens of interacting relations at once. Doing this with an LLM needs a representation where:
...
**PyGeoX** is that representation: a lightweight, object-oriented Python **DSL** that compiles geometric intent into symbolic constraints, aggregates them into a single error, and exposes **per-constraint residuals**. One engine serves two LLM workflows:
...
GCS problems are **under-constrained** (many valid placements up to similarity transforms), so you cannot grade with a single ground-truth coordinate file. PyGeoX instead asks: *how much does each constraint violate?* That continuous manifold is what makes **partial credit** possible, and what norm-based rewards like \(\exp(-\text{MSE})\) destroy when one outlier drives the whole signal to zero (**outlier gradient masking**).
...
<p align="center">
  <img src="docs/_static/readme/error_manifold_3.jpg" alt="Reward landscapes: global norm vs SAR" width="48%"/>
  <img src="docs/_static/plot3_avg_reward_vs_tokens.png" alt="Average reward vs tokens" width="48%"/>
</p>
<p align="center">
  <img src="docs/_static/distribution_comparison.png" alt="Distribution of exp(-SSE) vs SAR dense reward" width="70%"/>
</p>
<p align="center">
  <em>Top: SAR keeps signal on partially correct constructions; dense RL reaches higher reward with fewer tokens.</em><br/>
  <em>Bottom: SAR spreads reward over [0, 1]; exp(−SSE) collapses to a sparse near-binary distribution.</em>
</p>

<p align="center">
  <img src="docs/_static/readme/3obj_5rel_2extra_gen0244.png" alt="Hard-tier GCS example: NL description to verified coordinates" width="85%"/>
</p>
<p align="center"><em>Hard-tier example: natural language specifies objects and relations; a valid solution is a coordinate assignment with \(\|\mathbf{r}\|_2^2 &lt; 10^{-3}\).</em></p>
...
<p align="center"><em>Declarative scene (left) → symbolic constraints (middle) → aggregated error \(\mathcal{E}(\mathbf{u})\) and per-constraint residuals for optimization &amp; RL (right).</em></p>
...
Data generation and model calls read an `API_KEY` (and optional `OPEN_AI_BASE_URL`) from a `.env` at the repo root. For tutorials and API reference, see the [documentation](docs/_build/html/index.html).
...
**Data.** PyGeoX generates ~100k validated 4-tuples (*description · DSL code · per-constraint reward · image*), stratified Easy / Medium / Hard into two corpora: **PyGeoX-CodeGen-SFT** (NL → code) and **PyGeoX-GCS-RL** (NL → scene for rewards). Build them with [`data_generation_pipeline.py`](pygeox/synthetic/data_generation_pipeline.py) → [`prepare_data.ipynb`](model_training/prepare_data.ipynb), or download the released datasets from the **[🤗 Hugging Face Hub](https://huggingface.co/datasets/rafaelcabral96/PyGeoX)** — all subsets (`gcs-sft`, `codegen-sft`, `gcs-rl`) plus the `bench` and `wild` evaluation sets and the reward-ground-truth archives. Fetch and prepare everything with [`data/load_pygeox.py`](data/load_pygeox.py) (`python data/load_pygeox.py --root ./PyGeoX`), or stream a split via `load_dataset("rafaelcabral96/PyGeoX", "gcs-rl")`.

**Benchmarks.** [`PyGeoX-Bench`](model_testing/PygeoX-Bench/) (300 procedural, 100 × E/M/H) and [`PyGeoX-Wild`](model_testing/PyGeoX-Wild/) (86 OOD school-geometry problems). Outputs land in `generations/<model>/`, aggregates in `excells/`.
...
Composite reward: \(\mathcal{R} = \frac{w}{C}\sum_i e^{-r_i/T} + \mathbb{I}_{\text{suc}} R_{\text{bonus}} - \min(4, C_{\text{deg}})\).
...
```bibtex
@inproceedings{pygeox2026internalizing,
