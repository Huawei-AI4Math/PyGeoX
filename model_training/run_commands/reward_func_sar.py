...
USE_DENSE_REWARD = True  # Dense reward: SAR (r_pygeox/6 with R_success=0, max_overlap_penalty=0), outputs in [0, 1]
...
def llm_reward_function_new(
    ...
    def compute_single_reward(args: Tuple[str, str, str, bool]) -> Tuple[float, Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[SAR-DEBUG] Step1: r_format={r_format}, exec_err={exec_err}, has_points={points is not None}, has_circles={circles is not None}", flush=True)

            if r_format == 0.0:
                details["error"] = exec_err
                r_pygeox = WRONG_FORMAT_REWARD if dense_reward else 0.0
                print(f"[SAR-DEBUG] EARLY EXIT: r_format=0, error={exec_err}", flush=True)
                return float(r_pygeox), details

            if points is None or circles is None:
                details["error"] = "Points/Circles is None"
                r_pygeox = WRONG_FORMAT_REWARD if dense_reward else 0.0
                print(f"[SAR-DEBUG] EARLY EXIT: points/circles is None", flush=True)
                return float(r_pygeox), details
            ...
            print(f"[SAR-DEBUG] Step2: json_path={json_path}, exists={json_path.exists()}", flush=True)
            if not json_path.exists():
                 details["error"] = f"Source file not found: {source_path}"
                 val = WRONG_FORMAT_REWARD if dense_reward else 0.0
                 print(f"[SAR-DEBUG] EARLY EXIT: source file not found: {source_path}", flush=True)
            ...
            try:
                ...
                print(f"[SAR-DEBUG] Step3: scene created OK, calling reward_function...", flush=True)
                with contextlib.redirect_stdout(StringIO()), contextlib.redirect_stderr(StringIO()):
                    r_pygeox, details = scene.reward.reward_function(points, circles, R_success=0, max_overlap_penalty=0)
                print(f"[SAR-DEBUG] Step3: r_pygeox={r_pygeox}, details keys={list(details.keys()) if isinstance(details, dict) else 'NOT_DICT'}", flush=True)
                print(f"[SAR-DEBUG] Step3: SSE={details.get('SSE') if isinstance(details, dict) else 'N/A'}, full details={details}", flush=True)
                ...
                print(f"[SAR-DEBUG] Step3 EXCEPTION: {e}", flush=True)
            ...
            # Step 4: Total Reward = r_pygeox / 6.0 (SAR: dense-only, range [0, 6] -> [0, 1])
            if r_format == 1.0:
                if is_invalid(r_pygeox) or r_pygeox < 0.0:
                    total_reward = 0.0
                else:
                    total_reward = min(float(r_pygeox) / 6.0, 1.0)
                print(f"[SAR-DEBUG] Step4: r_pygeox={r_pygeox}, total_reward={total_reward}", flush=True)
            else:
                total_reward = 0.0
                print(f"[SAR-DEBUG] Step4: r_format!=1.0, total_reward=0", flush=True)
            details["R_total"] = total_reward
            print(f"[SAR-DEBUG] FINAL RETURN: total_reward={total_reward}", flush=True)
            return float(total_reward), details

        except BaseException as e:
            print(f"[SAR-DEBUG] GLOBAL EXCEPTION: {type(e).__name__}: {e}", flush=True)
            val = WRONG_FORMAT_REWARD if dense_reward else 0.0
            error_details = {"R_format": 0.0, "R_pygeox": val, "error": f"Global error: {str(e)}"}
    ...
    for idx in range(len(completions)):
        ...
        try:
            ...
            print(f"[SAR-DEBUG] Loop idx={idx}: reward={r}, error={d.get('error') if isinstance(d, dict) else 'N/A'}", flush=True)

        except BaseException as e:
            val = WRONG_FORMAT_REWARD if dense_reward else 0.0
            final_results[idx] = (float(val), {"error": f"Execution error: {type(e).__name__}: {str(e)}"})
            print(f"[SAR-DEBUG] Loop idx={idx}: EXCEPTION={e}", flush=True)
...
def reward_func(queries, prompts, labels):
    ...
    try:
        ...
        success_threshold = 0.9  # r_pygeox/6 > 0.9 means r_pygeox > 5.4 (90%+ constraints satisfied)
