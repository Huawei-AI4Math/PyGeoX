...
export WANDB_PROJECT="PyGeoX-RL-QWEN-8b-MSE-Mixed"
...
export RAY_object_spilling_config='{"type":"filesystem","params":{"directory_path":"/tmp/ray_spill_mse_mixed"}}'
# -----------------------------
...
# Only kill processes on THIS cluster's port (6380), not all Ray processes
# This allows multiple training scripts to run in parallel on different GPUs
MY_RAY_PORT=6380
MY_DASHBOARD_PORT=8265
...
# Start Ray cluster with reasonable object store memory (20GB per cluster for parallel runs)
# Setting it too high causes "Check failed: objects_valid" errors
unset RAY_ADDRESS
rm -rf /tmp/ray_spill_mse_mixed; mkdir -p /tmp/ray_spill_mse_mixed
...
python3 -m openrlhf.cli.train_ppo_ray \
   ...
   --remote_rm_url $PROJECT_ROOT/model_training/run_commands/reward_func_mse_sd.py \
   --save_path $PROJECT_ROOT/model_training/RL_checkpoint_8b_MSE_mixed \
   --ckpt_path $PROJECT_ROOT/model_training/RL_checkpoint_8b_MSE_mixed \
   --load_checkpoint \
   --save_steps 3 \
   --max_ckpt_num 25 \
   ...
   --wandb_project "PyGeoX-RL-LLM-MSE-MIXED-REWARD-8b"
