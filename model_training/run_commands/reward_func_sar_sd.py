...
USE_DENSE_REWARD = True  # Set to True for dense (0-10), False for sparse (0 or 1)
...
def llm_reward_function_new(
    ...
    def execute_and_extract(completion_text: str) -> Tuple[float, Any, Any, str]:
        ...
        try:
            ...
            return 0.0, None, None, f"Code Execution Error: {str(e)}"
    ...
    def compute_single_reward(args: Tuple[str, str, str, bool]) -> Tuple[float, Dict[str, Any]]:
        ...
        try:
            ...
            # Step 4: Total Reward
            if dense_reward:
                total_reward = r_pygeox if r_format == 1.0 else WRONG_FORMAT_REWARD 
                if is_invalid(total_reward) or total_reward < WRONG_FORMAT_REWARD or total_reward > 10.0:
                    total_reward = WRONG_FORMAT_REWARD
            else:
                total_reward = r_pygeox if r_format == 1.0 else 0.0
                if is_invalid(total_reward) or (total_reward != 0.0 and total_reward != 1.0):
                    total_reward = 0.0
    ...
    for idx in range(len(completions)):
        ...
        try:
            ...
            print("reward for sample", source_file[idx], " is", r)
            print("details", d)
            ...
            val = WRONG_FORMAT_REWARD if dense_reward else 0.0
            final_results[idx] = (float(val), {"error": f"Execution error: {str(e)}"})
    ...
    for i, res in enumerate(final_results):
        ...
        # Final sanity checks - ensure valid float in expected range
        try:
            r = float(r)
            if dense_reward:
                if is_invalid(r) or r < WRONG_FORMAT_REWARD or r > 10.0:
                     r = float(WRONG_FORMAT_REWARD)
            else:
                if is_invalid(r) or (r != 0.0 and r != 1.0):
                     r = 0.0
        except (ValueError, TypeError):
            r = float(WRONG_FORMAT_REWARD) if dense_reward else 0.0
...
def reward_func(queries, prompts, labels):
    ...
    try:
        ...
        success_threshold = 9.0 if USE_DENSE_REWARD else 1.0
        ...
        print(f"CRITICAL ERROR in reward_func: {e}")
