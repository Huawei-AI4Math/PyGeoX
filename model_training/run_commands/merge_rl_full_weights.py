#!/usr/bin/env python
"""
Merge full-weight RL (GRPO) DeepSpeed ZeRO checkpoint into a single HuggingFace model.
Uses the checkpoint's zero_to_fp32 to consolidate ZeRO-3 shards, then loads into base qwen3-8b and saves.
"""
import os
import sys
...
CHECKPOINT_PARENT = "YOUR_CHECKPOINT_PATH/_actor"  # e.g. "/path/to/project/model_training/RL_checkpoint/_actor"
CHECKPOINT_TAG = "global_step16"  # the checkpoint step tag to merge
SAVE_PATH = "YOUR_SAVE_PATH"  # e.g. "/path/to/project/model_training/merged/qwen3-8b-RL-step16"

# Import zero_to_fp32 from the checkpoint's _actor folder (script lives next to global_step* dirs)
...
def main():
    print(f"📦 Merging full-weight RL checkpoint: {CHECKPOINT_PARENT} / {CHECKPOINT_TAG}")
