...
# ============================================================================
# MS-Swift SFT Training Script with SSE-Weighted Dataset (Full Parameter Fine-Tuning)
# 
# This script performs Supervised Fine-Tuning (SFT) using ModelScope Swift
# framework with full parameter training (not LoRA).
# Uses SSE-weighted dataset (exp(-SSE) weights for sharper distinction).
#
# Usage:
#   bash sft_exp_SSE.sh
#   or
...
# ==================== Configuration ====================
# GPU Configuration (CHANGED from 3 to avoid conflict with sft_dense.sh)
...
export WANDB_PROJECT="qwen3-8b-SFT-SSE"
...
# Uses SSE-weighted dataset with exp(-SSE) weights for sharper distinction
DATASET_PATH="${PROJECT_ROOT}/model_training/old_data/rejection_sampling/rs_sse_weighted_exp_sse_fixed.jsonl"
OUTPUT_DIR="${PROJECT_ROOT}/model_training/sft_training/SFT_SSE"
...
RUN_NAME="SFT_SSE_run1"  # Wandb run name (for tracking experiments)

# Wandb Tags (optional, for filtering runs)
# Set as environment variable for wandb to pick up
# export WANDB_TAGS="dense,sft,qwen3-8b"

# Wandb Notes (optional, for run description)
# Set as environment variable for wandb to pick up
# export WANDB_NOTES="SFT training with SSE-weighted dataset (exp(-SSE))"
# ========================================================
