...
def llm_reward_function_new(
    ...
    def compute_single_reward(args: Tuple[str, str, str, bool]) -> Tuple[float, Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[MSE-MIX-DEBUG] Step1: r_format={r_format}, exec_err={exec_err}, has_points={points is not None}, has_circles={circles is not None}", flush=True)

            if r_format == 0.0:
                details["error"] = exec_err
                r_pygeox = WRONG_FORMAT_REWARD if dense_reward else 0.0
                print(f"[MSE-MIX-DEBUG] EARLY EXIT: r_format=0, error={exec_err}", flush=True)
                return float(r_pygeox), details

            if points is None or circles is None:
                details["error"] = "Points/Circles is None"
                r_pygeox = WRONG_FORMAT_REWARD if dense_reward else 0.0
                print(f"[MSE-MIX-DEBUG] EARLY EXIT: points/circles is None", flush=True)
                return float(r_pygeox), details
            ...
            print(f"[MSE-MIX-DEBUG] Step2: json_path={json_path}, exists={json_path.exists()}", flush=True)
            if not json_path.exists():
                 details["error"] = f"Source file not found: {source_path}"
                 val = WRONG_FORMAT_REWARD if dense_reward else 0.0
                 print(f"[MSE-MIX-DEBUG] EARLY EXIT: source file not found: {source_path}", flush=True)
            ...
            try:
                ...
                print(f"[MSE-MIX-DEBUG] Step3: scene created OK, calling reward_function...", flush=True)
                with contextlib.redirect_stdout(StringIO()), contextlib.redirect_stderr(StringIO()):
                    r_pygeox, details = scene.reward.reward_function(points, circles)
                print(f"[MSE-MIX-DEBUG] Step3: r_pygeox={r_pygeox}, details keys={list(details.keys()) if isinstance(details, dict) else 'NOT_DICT'}", flush=True)
                print(f"[MSE-MIX-DEBUG] Step3: SSE={details.get('SSE') if isinstance(details, dict) else 'N/A'}, full details={details}", flush=True)
                ...
                print(f"[MSE-MIX-DEBUG] Step3 EXCEPTION: {e}", flush=True)
            ...
            # Step 4: Mixed Reward = 0.6 * exp(-SSE/10) + 0.4 * I[r_pygeox > 9]
            dense_part = 0.0
            sparse_part = 0.0
            if r_format == 1.0:
                sse = details.get("SSE")
                print(f"[MSE-MIX-DEBUG] Step4: r_format={r_format}, SSE={sse}, r_pygeox={r_pygeox}", flush=True)
                if sse is not None and isinstance(sse, (int, float)) and not math.isnan(float(sse)):
                    dense_part = math.exp(-float(sse) / 10.0)
                # Sparse indicator: 1 if scene reward > 9
                if isinstance(r_pygeox, (int, float)) and not is_invalid(r_pygeox) and r_pygeox > 9.0:
                    sparse_part = 1.0
                total_reward = 0.6 * dense_part + 0.4 * sparse_part
                print(f"[MSE-MIX-DEBUG] Step4: dense={dense_part:.4f}, sparse={sparse_part}, total=0.6*{dense_part:.4f}+0.4*{sparse_part}={total_reward:.4f}", flush=True)
            else:
                total_reward = 0.0
                print(f"[MSE-MIX-DEBUG] Step4: r_format!=1.0, total_reward=0", flush=True)
            if is_invalid(total_reward) or total_reward < 0.0 or total_reward > 1.0:
                print(f"[MSE-MIX-DEBUG] Step4: SANITY FAIL total_reward={total_reward}, setting to 0", flush=True)
                total_reward = 0.0
            details["R_total"] = total_reward
            details["R_dense_part"] = dense_part
            details["R_sparse_part"] = sparse_part
            print(f"[MSE-MIX-DEBUG] FINAL RETURN: total_reward={total_reward}", flush=True)
            return float(total_reward), details

        except BaseException as e:
            print(f"[MSE-MIX-DEBUG] GLOBAL EXCEPTION: {type(e).__name__}: {e}", flush=True)
            val = WRONG_FORMAT_REWARD if dense_reward else 0.0
    ...
    for idx in range(len(completions)):
        ...
        try:
            ...
            print(f"[MSE-MIX-DEBUG] Loop idx={idx}: reward={r}, error={d.get('error') if isinstance(d, dict) else 'N/A'}", flush=True)

        except BaseException as e:
            val = WRONG_FORMAT_REWARD if dense_reward else 0.0
            final_results[idx] = (float(val), {"error": f"Execution error: {type(e).__name__}: {str(e)}"})
            print(f"[MSE-MIX-DEBUG] Loop idx={idx}: EXCEPTION={type(e).__name__}: {e}", flush=True)
